      ...
      <aside className="w-full md:w-64 shrink-0">
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
          <div className="p-6 bg-blue-600 flex flex-col items-center text-center">
            ...
            <h2 className="mt-3 text-lg font-bold text-white">{user?.name || t('common.guest', 'Guest User')}</h2>
            <p className="text-blue-100 text-sm">{user?.email || 'guest@example.com'}</p>
          </div>
          
          <nav className="p-2 space-y-1">
            <button className="flex items-center gap-3 w-full p-3 text-sm font-semibold text-blue-600 bg-blue-50 rounded-xl transition-colors cursor-pointer text-left rtl:text-right">
              <UserIcon size={18} /> {t('profile.personal_info')}
            </button>
            <button className="flex items-center gap-3 w-full p-3 text-sm font-semibold text-gray-600 hover:bg-gray-50 rounded-xl transition-colors cursor-pointer text-left rtl:text-right">
              <MapPin size={18} /> {t('profile.saved_addresses')}
            </button>
            <button className="flex items-center gap-3 w-full p-3 text-sm font-semibold text-gray-600 hover:bg-gray-50 rounded-xl transition-colors cursor-pointer text-left rtl:text-right">
              <CreditCard size={18} /> {t('profile.payment_methods')}
            </button>
            <button className="flex items-center gap-3 w-full p-3 text-sm font-semibold text-gray-600 hover:bg-gray-50 rounded-xl transition-colors cursor-pointer text-left rtl:text-right">
            ...
            <button 
              onClick={logout}
              className="flex items-center gap-3 w-full p-3 text-sm font-semibold text-red-600 hover:bg-red-50 rounded-xl transition-colors cursor-pointer text-left rtl:text-right"
            >
